...
def deal(train):
    ...
    id2word.save('./LDA/tmp/deerwester.dict')  #保存词典
    corpora.MmCorpus.serialize('./LDA/tmp/deerwester.mm', corpus)  #保存corpus
...
def run(corpus_1, id2word_1, num, texts):
    ...
    perplex = lda_model.log_perplexity(
        corpus_1)  # a measure of how good the model is. lower the better.
    # 一致性
...
if __name__ == '__main__':
    train = infile('./LDA/all.csv')
    id2word, texts, corpus = deal(train)
    lda = run(corpus, id2word, 6, texts)
    topic_list = lda.print_topics()
    f = open('./LDA/RESULT.txt', 'w', encoding='utf-8')
    for t in topic_list:
        f.write(' '.join(str(s) for s in t) + '\n')
    f.close()
    save_visual(lda, corpus, id2word, 'ALL')

...
def seg_sentence(sentence):
    ...
    jieba.load_userdict('./LDA/自建词表.txt')  #加载自建词表
    ...
    stopwords = stopwordslist('./LDA/停用词表.txt')  # 这里加载停用词的路径
    synwords = synwordslist('./LDA/近义词表.txt')  #这里加载近义词的路径
    outstr = ''
...
if __name__ == "__main__":
    inputs = open('./LDA/all.csv', 'r', encoding='utf-8')
    outputs = open('./LDA/result.txt', 'w', encoding='utf-8')

    pool = ThreadPool()
    alls = pool.map(seg_sentence,
                    inputs)  #多线程和普通的列表嵌套不一样，是直接将列表里面的元素取出来了。而且输出是二次嵌套列表
    pool.close()
    pool.join()

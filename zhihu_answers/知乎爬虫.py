...
def split_sentence(text, punctuation_list='!?。！？'):
    ...
    sentence_with_index = {i: sent
                           for i, sent in enumerate(sentence_set)
                           }  #dict(zip(sentence_set, range(len(sentences))))
...
def get_sentence_with_words_weight(tfidf_matrix):
    ...
    for key in sentence_with_words_weight.keys():
        ...
        sentence_with_words_weight[key] = (x - min_weight) / (max_weight -
                                                              min_weight)
...
def ranking_base_on_weigth(sentence_with_words_weight,
    ...
    sort_sent_weight = sorted(sentence_weight.items(),
                              key=lambda d: d[1],
                              reverse=True)
...
def parse_data(html):
    ...
    comments = [[
        'name', 'gender', 'follower_count', 'voteup', 'cmt_count', 'content',
        'character_num', 'emotion1', 'relief', 're1'
    ]]
    ...
    try:
        for item in json_data:
            ...
            if len(raw) > 400:
                sentence_set, sentence_with_index = split_sentence(
                    raw, punctuation_list='!?。！？')
                tfidf_matrix = get_tfidf_matrix(sentence_set, stop_word)
                sentence_with_words_weight = get_sentence_with_words_weight(
                    tfidf_matrix)
                sentence_with_position_weight = get_sentence_with_position_weight(
                    sentence_set)
                sentence_score = get_similarity_weight(tfidf_matrix)
                sort_sent_weight = ranking_base_on_weigth(
                    sentence_with_words_weight,
                    sentence_with_position_weight,
                    sentence_score,
                    feature_weight=[1, 1, 1])
                r = 300 / len(raw)
                summarization = get_summarization(sentence_with_index,
                                                  sort_sent_weight,
                                                  topK_ratio=r)
